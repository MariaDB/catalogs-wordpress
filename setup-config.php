...
<p><?php _e( 'Welcome to WordPress. Your hosting provider is using this cool feature called MariaDB Catalog. So while you can provide your own database configuration, you don\'t need to. It will create the following information for you.' ); ?></p>
<ol>
...
<form method="post" action="setup-config.php?step=2">
	...
	<table class="form-table" role="presentation">
		<tr>
			...
			<td><input name="dbname" id="dbname" type="text" aria-describedby="dbname-desc" size="25" value="<?php echo $config['dbname']; ?>" placeholder="wordpress"<?php echo $autofocus; ?> />
			<p id="dbname-desc"><?php _e( 'The name of the database you want to use with WordPress.' ); ?></p></td>
		</tr>
		<tr>
			<th scope="row"><label for="uname"><?php _e( 'Username' ); ?></label></th>
			<td><input name="uname" id="uname" type="text" aria-describedby="uname-desc" size="25" value="<?php echo $config['username']; ?>" placeholder="<?php echo htmlspecialchars( _x( 'username', 'example username' ), ENT_QUOTES ); ?>" />
		...
		<tr>
			...
			<td>
				<div class="wp-pwd">
					<input name="pwd" id="pwd" type="password" class="regular-text" data-reveal="1" aria-describedby="pwd-desc" size="25" value="<?php echo $config['password']; ?>" placeholder="<?php echo htmlspecialchars( _x( 'password', 'example password' ), ENT_QUOTES ); ?>" autocomplete="off" spellcheck="false" />
		...
		<tr>
			...
			<td><input name="dbhost" id="dbhost" type="text" aria-describedby="dbhost-desc" size="25" value="<?php echo $config['host']; ?>" />
			<p id="dbhost-desc">
			<?php
...
<input name="noapi" type="hidden" value="1" /><?php } ?>
	...
	case 2:
		...
		$step_1  = 'setup-config.php?step=1';
		$install = wp_guess_url() . '/wp-admin/install.php';
		if ( isset( $_REQUEST['noapi'] ) ) {
			$step_1 .= '&amp;noapi';
		}
		...
		// Create catalog/database based on provided information
		MariaDB_Catalog::get_api()->create( $dbname, $uname, $pwd );
		...
		$wpdb = new MariaDB_DB( DB_USER, DB_PASSWORD, DB_NAME, DB_HOST );
